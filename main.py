...
def run_diagnostic_check() -> Dict[str, Any]:
    """Run a quick diagnostic check on the application."""
    diagnostics = {
    ...
    try:
        ...
        required_modules = [
            "streamlit", "pandas", "numpy", "nltk",
            "vaderSentiment", "requests", "plotly"
        ]
        diagnostics["checks_passed"] += 1

        # Check 2: Session state initialization
        if 'analysis_history' in st.session_state:
            diagnostics["checks_passed"] += 1
        else:
            diagnostics["warnings"].append("Session state not fully initialized")
        ...
        try:
            ...
            diagnostics["issues"].append("NLTK data not downloaded")

        # Check 4: YouTube API availability (simulated)
        diagnostics["checks_passed"] += 1

        # Check 5: Sentiment analyzer
        try:
            analyzer = SentimentIntensityAnalyzer()
            test_score = analyzer.polarity_scores("test")
            if 'compound' in test_score:
                diagnostics["checks_passed"] += 1
        except Exception:
            diagnostics["issues"].append("Sentiment analyzer not working")

        # Check 6: Plotly availability
        diagnostics["checks_passed"] += 1
        ...
        # Check 8: Memory usage check (simulated)
        if len(st.session_state.get('analysis_history', [])) > 100:
            diagnostics["warnings"].append("Large history may impact performance")
        diagnostics["checks_passed"] += 1

        # Check 9: Streamlit configuration
        diagnostics["checks_passed"] += 1

        # Check 10: Error handling
        diagnostics["checks_passed"] += 1

        # Check 11: Video processing capability
        diagnostics["checks_passed"] += 1

        # Check 12: Export functionality
        diagnostics["checks_passed"] += 1

        # Overall status
        success_rate = (diagnostics["checks_passed"] /
                       diagnostics["total_checks"]) * 100
        if success_rate < 70:
            diagnostics["status"] = "🔴 Needs Attention"
        elif success_rate < 90:
            diagnostics["status"] = "🟡 Minor Issues"
        else:
...
class FilmSpecificScorer:
    ...
    def calculate_unique_film_score(self, analysis_results: Dict, film_data: Dict) -> Dict:
        ...
        text = (film_data.get('synopsis', '') + ' ' +
                film_data.get('transcript', '')).lower()
        ...
        narrative_score = self._score_narrative(
            analysis_results.get('narrative_structure', {}), text)
        emotional_score = self._score_emotional(
            analysis_results.get('emotional_arc', {}))
        character_score = self._score_characters(
            analysis_results.get('character_analysis', {}), text)
        cultural_score = analysis_results.get(
            'cultural_context', {}).get('relevance_score', 0.0)
        ...
        drama_keywords = ['drama', 'urban drama', 'black cinema', 'black', 'urban']
        if any(g in detected_genre for g in drama_keywords):
        ...
        philosophical_bonus = self._calculate_philosophical_bonus(
            text, cultural_score)
    ...
    def _score_characters(self, ca: Dict, text: str) -> float:
        ...
        mentions = text.count(" he ") + text.count(" she ") + \
            text.count(" his ") + text.count(" her ")
    ...
    def _score_technical(self, ns: Dict, text: str) -> float:
        ...
        dialogue_density = len(re.findall(r'\b[A-Z][a-z]+:', text)) / \
...
class SmartGenreDetector:
    ...
    def detect_genre(self, text: str, existing_genre: Optional[str] = None) -> Dict:
        ...
        secondary_genres = [
            g for g, s in genre_scores.items()
            if s >= top_score * 0.5 and g != top_genre
        ]
...
class CulturalContextAnalyzer:
    ...
    def analyze_cultural_context(self, film_data: Dict) -> Dict:
        ...
        max_possible = sum(
            len(theme_data['keywords']) * theme_data.get('weight', 1.0)
            for theme_data in self.cultural_themes.values()
        )
        relevance_score = min(1.0, total_weighted_matches / (max_possible * 0.15))

        primary_themes = sorted(
            theme_scores.items(), key=lambda x: x[1], reverse=True)[:2]
        ...
        for theme in primary_themes:
            if theme in theme_details:
                philosophical_insights.append(
                    theme_details[theme]['philosophical_aspect'])
...
class FilmAnalysisEngine:
        ...
        try:
            ...
            if len(analysis_text.strip()) < 20:
                ...
                analysis_results = self._perform_comprehensive_analysis(
                    analysis_text, film_data)
                scoring_result = self.film_scorer.calculate_unique_film_score(
                    analysis_results, film_data)
                results = self._generate_enhanced_review(
                    film_data, analysis_results, scoring_result)
    ...
    def _perform_comprehensive_analysis(self, text: str, film_data: Dict) -> Dict:
        ...
        # Cultural analysis
        cultural_result = self.cultural_analyzer.analyze_cultural_context(
            film_data)
    ...
    def _analyze_characters(self, text: str) -> Dict:
        ...
        words = nltk.word_tokenize(text)
        potential_characters = len(
            [w for w in words if w.istitle() and len(w) > 1])
    ...
    def _generate_enhanced_review(self, film_data: Dict, analysis_results: Dict, scoring_result: Dict) -> Dict:
        ...
        cinematic_scores = self._map_to_cinematic_categories(
            component_scores, genre_result)
    ...
    def _generate_philosophical_summary(self, film_data: Dict, score: float, genre_result: Dict, cultural_context: Dict) -> str:
        ...
        philosophical_aspect = genre_info.get(
            'philosophical_aspect', 'human experience')
    ...
    def _generate_enhanced_strengths(self, analysis_results: Dict, cinematic_scores: Dict, cultural_context: Dict) -> List[str]:
        ...
        if cinematic_scores.get('story_narrative', 0) >= 4.0:
            strengths.append(
                "Strong narrative structure with compelling storytelling")

        # Check character development
        if cinematic_scores.get('performance', 0) >= 4.0:
            strengths.append(
                "Well-developed characters with depth and authenticity")
        ...
        if cinematic_scores.get('technical_craft', 0) >= 3.5:
            strengths.append(
                "Solid technical execution and production values")
    ...
    def _generate_enhanced_weaknesses(self, analysis_results: Dict, cinematic_scores: Dict) -> List[str]:
        ...
        min_category = min(cinematic_scores.items(),
                           key=lambda x: x[1])[0] if cinematic_scores else ""
        ...
        if min_score < 3.0:
            ...
            weakness_category = category_map.get(
                min_category, min_category.replace('_', ' '))
    ...
    def _generate_enhanced_recommendations(self, analysis_results: Dict, cinematic_scores: Dict, cultural_context: Dict) -> List[str]:
        ...
        if cinematic_scores.get('story_narrative', 0) < 3.5:
            recommendations.append(
                "Develop narrative complexity with subplots")
        ...
        if cultural_context.get('is_culturally_relevant'):
            recommendations.append(
                "Leverage cultural themes for deeper resonance")

        # Technical recommendations
        if cinematic_scores.get('technical_craft', 0) < 3.5:
            recommendations.append(
                "Enhance production values with focused resources")

        if not recommendations:
            recommendations.append(
                "Continue developing your distinctive voice")
            recommendations.append(
                "Explore collaborations to enhance production scope")
    ...
    def _generate_enhanced_audience_analysis(self, analysis_results: Dict, genre_result: Dict, cultural_context: Dict) -> Dict:
        ...
        audiences = audience_mapping.get(
            genre, ["General film audiences", "Festival attendees"])
        ...
        engagement_score = random.uniform(
            0.6, 0.9) if len(audiences) > 3 else random.uniform(0.4, 0.7)
    ...
    def _generate_philosophical_insights(self, film_data: Dict, analysis_results: Dict) -> List[str]:
        ...
        existential_keywords = ['death', 'life',
                                'meaning', 'existence', 'purpose']
        ...
        social_keywords = ['society', 'justice',
                           'equality', 'power', 'freedom']
        ...
        psychological_keywords = ['mind', 'memory',
                                  'identity', 'consciousness', 'dream']
        ...
        # Cultural insights
        cultural_insights = analysis_results.get(
            'cultural_context', {}).get('philosophical_insights', [])
...
# --------------------------
# ENHANCED HISTORY ANALYTICS PAGE (MISSING CLASS)
# --------------------------


class EnhancedHistoryAnalyticsPage:
    """Enhanced analytics page for history and trends."""

    def __init__(self, persistence: PersistenceManager):
        self.persistence = persistence

    def show(self) -> None:
        """Display the enhanced analytics page."""
        st.header("📈 Advanced Analytics & Historical Trends")

        # Get analytics data
        df = self.persistence.get_analytics_data()

        if df is None or df.empty:
            st.info("No analysis history available. Analyze some films first!")
            return
        ...
        with col2:
            st.metric("Average Score", f"{df['overall_score'].mean():.1f}/5.0")
        with col3:
            st.metric("High Scores (≥4.0)", len(df[df['overall_score'] >= 4.0]))
        with col4:
            st.metric("Cultural Films", len(
                df[df['cultural_relevance'] > 0.5]))

        # Tabs for different analytics views
        tab1, tab2, tab3 = st.tabs(
            ["📊 Score Trends", "🎭 Genre Analysis", "📅 Temporal Patterns"])

        with tab1:
            self._show_score_trends(df)

        with tab2:
            self._show_genre_analysis(df)

        with tab3:
            self._show_temporal_patterns(df)

    def _show_score_trends(self, df: pd.DataFrame) -> None:
        """Display score trends over time."""
        st.subheader("Score Distribution Over Time")

        # Create time series chart
        df_sorted = df.sort_values('timestamp')
        fig = px.line(df_sorted, x='timestamp', y='overall_score',
                      title='Film Scores Over Time',
        ...
        fig2 = px.histogram(df, x='overall_score', nbins=10,
                            title='Score Distribution',
                            labels={'overall_score': 'Score', 'count': 'Number of Films'})
        fig2.update_layout(height=300)
        st.plotly_chart(fig2, use_container_width=True)

    def _show_genre_analysis(self, df: pd.DataFrame) -> None:
        """Display genre analysis."""
        st.subheader("Genre Performance Analysis")

        if 'detected_genre' not in df.columns:
            st.info("No genre data available")
            return

        # Genre distribution
        genre_counts = df['detected_genre'].value_counts().head(10)
        ...
        # Genre performance (average score per genre)
        genre_performance = df.groupby('detected_genre')[
            'overall_score'].mean().sort_values(ascending=False).head(10)

        col1, col2 = st.columns(2)
        with col1:
            st.write("**Top Performing Genres:**")
            for genre, score in genre_performance.head(5).items():
                st.write(f"• {genre}: {score:.1f}/5.0")

        with col2:
            st.write("**Most Common Genres:**")
            for genre, count in genre_counts.head(5).items():
                st.write(f"• {genre}: {count} films")

    def _show_temporal_patterns(self, df: pd.DataFrame) -> None:
        """Display temporal patterns in analysis."""
        st.subheader("Temporal Analysis Patterns")

        # Analysis by time of day
        if 'time_of_day' in df.columns:
            time_dist = df['time_of_day'].value_counts().sort_index()
            fig = px.bar(time_dist, x=time_dist.index, y=time_dist.values,
                         title='Analyses by Hour of Day',
                         labels={'x': 'Hour', 'y': 'Number of Analyses'})
        ...
        fig2 = px.line(daily_counts, x='date', y='count',
                       title='Daily Analysis Frequency',
                       labels={'date': 'Date', 'count': 'Analyses'})
        fig2.update_layout(height=300)
        st.plotly_chart(fig2, use_container_width=True)
...
class EnhancedFilmAnalysisInterface:
    ...
    def show_dashboard(self) -> None:
        ...
        if st.session_state.get('show_results_page') and st.session_state.get('current_results_display'):
            self._display_enhanced_film_results(
                st.session_state.current_results_display)
        ...
        with col1:
            st.metric("Total Films", stats['total_films'],
                      help="Total number of films analyzed")
        with col2:
        ...
        with col4:
            if stats['total_films'] > 0:
                st.metric("Cultural Films", stats['cultural_films'],
                          help="Films with significant cultural relevance")
        with col5:
            if stats['total_films'] > 0:
                st.metric("Top Genre", stats['top_genre'][:15],
                          help="Most frequently detected genre")
        with col6:
            if stats['total_films'] > 0:
                st.metric("Analysis Rate", f"{stats['analysis_rate']}/day",
            ...
            with col1:
                ...
                if stats['recent_analyses']:
                    for analysis in stats['recent_analyses'][:3]:
                        st.write(
                            f"• {analysis['title'][:20]}: {analysis['score']}/5.0")
                else:
        ...
        tab1, tab2, tab3 = st.tabs(
            ["🎥 YouTube Analysis", "📝 Manual Entry", "📊 CSV Batch"])
    ...
    def _show_youtube_analysis(self) -> None:
        ...
        st.markdown(
            "Analyze films from YouTube videos by providing a video URL or ID.")
        ...
        with col2:
            ...
            analyze_button = st.button(
                "🎬 Analyze Video", type="primary", width='stretch')
        ...
        if analyze_button and youtube_input:
            with st.spinner("🔄 Extracting and analyzing video content..."):
                try:
                    ...
                    if not video_id:
                        st.error(
                            "Invalid YouTube URL or Video ID. Please check your input.")
                        return
                    ...
                    try:
                            ...
                            try:
                                ...
                                for transcript in transcript_list:
                                    ...
                                    transcript_text = " ".join(
                                        [entry['text'] for entry in transcript_data])
                                    st.info(
                                        f"✅ Found transcript in {transcript.language}")
                                    break
                            except Exception as e:
                                st.warning(
                                    f"⚠️ Could not extract transcript: {str(e)[:100]}")
                                transcript_text = ""
                        ...
                        st.warning(
                            f"⚠️ Could not extract transcript: {str(e)[:100]}")
                        st.info(
                            "Continuing analysis with available metadata only...")
                    ...
                    if not transcript_text.strip():
                        st.warning(
                            "No transcript available. Analysis will be based on video metadata only.")
                    ...
                    st.session_state.current_video_title = video_info.get(
                        'title', 'Unknown')

                    # Display success message
                    st.success(
                        f"✅ Successfully analyzed: {video_info.get('title', 'Unknown')}")
    ...
    def _show_manual_analysis(self) -> None:
        ...
        with st.form("manual_analysis_form"):
            ...
            with col1:
                title = st.text_input(
                    "Film Title *", placeholder="Enter film title", key="manual_title")
                director = st.text_input(
                    "Director", placeholder="Director's name", key="manual_director")
                writer = st.text_input(
                    "Writer", placeholder="Writer's name", key="manual_writer")

            with col2:
                duration = st.text_input(
                    "Duration", placeholder="e.g., 120m, 2h 15m", key="manual_duration")
                genre = st.text_input(
                    "Genre (optional)", placeholder="e.g., Drama, Comedy", key="manual_genre")
                year = st.number_input(
                    "Year", min_value=1900, max_value=2100, value=2023, key="manual_year")
            ...
            submitted = st.form_submit_button(
                "🎬 Analyze Film", type="primary", width='stretch')

            if submitted:
                if not title or not synopsis:
                    st.error(
                        "Please provide at least a film title and synopsis.")
                    return
    ...
    def _show_csv_interface(self) -> None:
        ...
        with col2:
            ...
            analyze_batch = st.button(
                "📦 Analyze Batch", type="primary", width='stretch', disabled=not uploaded_file)
            ...
            with st.spinner("📊 Processing batch analysis..."):
                try:
                    ...
                    if 'title' not in df.columns or 'synopsis' not in df.columns:
                        st.error(
                            "CSV must contain 'title' and 'synopsis' columns.")
                        return
                    ...
                    st.info(
                        f"📈 Average score: {np.mean([r['overall_score'] for r in results]):.1f}/5.0")
    ...
    def _show_enhanced_top_films_section(self) -> None:
        ...
        if top_films:
            ...
            st.caption(
                "*Films demonstrating exceptional artistic merit and cultural resonance*")
            ...
            for idx, film_data in enumerate(top_films[:3]):
                with cols[idx]:
                    ...
                    genre = analysis.get('genre_insights', {}).get(
                        'primary_genre', 'Unknown')
                    if isinstance(genre, dict):
                        genre = genre.get('primary_genre', 'Unknown')

                    cultural_score = analysis.get(
                        'cultural_insights', {}).get('relevance_score', 0)
                    ...
                    if st.button(f"🔍 Deep Analysis", key=f"view_top_{idx}", width='stretch'):
                        ...
                        st.session_state.current_analysis_id = film_data.get(
                            'film_id')
                        st.session_state.show_results_page = True
                        st.rerun()
    ...
    def _display_enhanced_batch_results(self, batch_results: List[Dict]) -> None:
        ...
        fig = go.Figure(data=[go.Histogram(
            x=scores, nbinsx=10, marker_color='#667eea')])
        fig.update_layout(
            title='Score Distribution',
            xaxis_title='Score',
            yaxis_title='Count',
        ...
        top_films = sorted(batch_results, key=lambda x: x[
                           'overall_score'], reverse=True)[:3]
        ...
        for i, film in enumerate(top_films):
            with st.expander(f"{i+1}. {film['title'][:30]} - {film['overall_score']}/5.0"):
                ...
                with col1:
                    ...
                    st.write(
                        f"**Cultural Relevance:** {film['cultural_relevance']:.0%}")
        ...
        with col2:
            if st.button("✨ Save to Projects", width='stretch'):
                project_name = st.text_input(
                    "Project Name:", value=f"Batch Analysis {datetime.now().strftime('%Y-%m-%d')}")
                ...
                if project_name:
                    ...
                    st.success(
                        f"✅ Saved {len(batch_results)} films to project: {project_name}")
    ...
    def _get_enhanced_statistics(self) -> Dict:
        ...
        if len(history) > 1:
            dates = [datetime.fromisoformat(h['timestamp']).date()
                     for h in history]
    ...
    def _display_enhanced_film_results(self, results: Dict) -> None:
        ...
        if st.session_state.current_analysis_id:
            stored_result = self.persistence.load_results(
                st.session_state.current_analysis_id)
            if stored_result:
                film_data = stored_result['film_data']

        # Display film title with philosophical context
        film_title = film_data.get(
            'title', results.get('film_title', 'Unknown Film'))
        philosophical_insights = results.get('philosophical_insights', [])
        primary_insight = philosophical_insights[
            0] if philosophical_insights else "Cinematic Exploration"
            ...
            with info_col:
                ...
                with yt_col1:
                    ...
                    if st.button("📺 Open YouTube", key="open_yt", width='stretch'):
                        st.markdown(
                            f'<meta http-equiv="refresh" content="0; url={youtube_url}">', unsafe_allow_html=True)
        ...
        if video_id:
            detail_tabs = st.tabs(["🎬 Film Info", "🎥 Video Analysis",
                                  "🧠 Philosophical", "🤖 AI Tools", "📊 Analytics"])
        else:
            detail_tabs = st.tabs(["🎬 Film Information", "🧠 Philosophical Insights",
                                  "🤖 AI Enhancement", "📊 Deep Analytics"])
        ...
        if video_id:
            ...
            with detail_tabs[1]:
                self._display_video_analysis_section(
                    film_data, results, video_id)
        ...
        with st.expander("📈 **Advanced Score Breakdown & Distribution**",
                         expanded=st.session_state.get('show_breakdown', False)):
        ...
        if synopsis_analysis.get('length', 0) > 0:
            ...
            with col2:
                st.metric("Sentiment",
                          synopsis_analysis['emotional_tone'].title())
            with col3:
                st.metric("Complexity",
                          synopsis_analysis['complexity'].title())

            if synopsis_analysis.get('key_themes'):
                st.write("**Key Themes:** " +
                         ", ".join(synopsis_analysis['key_themes']))
    ...
    def _display_film_information(self, film_data: Dict, results: Dict) -> None:
        ...
        with col2:
            ...
            if isinstance(genre_insights, dict) and 'primary_genre' in genre_insights:
                st.write(
                    f"**Primary Genre:** {genre_insights['primary_genre']}")
                if genre_insights.get('confidence'):
                    st.write(
                        f"**Confidence:** {genre_insights['confidence']}%")
    ...
    def _display_video_analysis_section(self, film_data: Dict, results: Dict, video_id: str) -> None:
        ...
        if transcript:
            ...
            with st.expander("🔍 View Key Video Moments", expanded=False):
                if word_count > 500:
                    ...
                    key_lines = [line.strip()
                                 for line in lines[:10] if len(line.strip()) > 50]
                    for i, line in enumerate(key_lines[:5]):
                        st.write(f"**Moment {i+1}:** {line[:200]}...")
                else:
                    st.write("Transcript preview:")
                    st.text(transcript[:500] + "..." if len(
                        transcript) > 500 else transcript)

            # Sentiment from transcript
            if transcript:
                sentiment = self.analyzer.sentiment_analyzer.polarity_scores(
                    transcript)
            ...
            st.info(
                "No transcript available for this video. Score based on metadata only.")
        ...
        if cinematic_scores.get('performance', 0) >= 4.0:
            score_moments.append(
                "Watch character interactions and performances (around 25% mark)")

        if cinematic_scores.get('story_narrative', 0) >= 4.0:
            score_moments.append(
                "Observe narrative structure and plot development (mid-point)")

        if cinematic_scores.get('visual_vision', 0) >= 4.0:
            score_moments.append(
                "Notice cinematography and visual composition (throughout)")

        if cinematic_scores.get('sound_design', 0) >= 4.0:
            score_moments.append(
                "Listen to audio design and music integration (climax scene)")
    ...
    def _display_philosophical_insights(self, results: Dict) -> None:
        ...
        if philosophical_insights or cultural_insights.get('philosophical_insights'):
            ...
            st.info(
                "No specific philosophical insights detected. This film appears to focus on direct narrative storytelling.")
    ...
    def _display_ai_enhancements(self, results: Dict) -> None:
        ...
        if ai_suggestions:
            ...
            st.caption(
                "Suggested tools for deeper analysis and improved scoring")
            ...
            for suggestion in ai_suggestions:
                with st.expander(f"{suggestion['tool']} - {suggestion['purpose']}"):
                    ...
                    st.write(
                        f"**Implementation:** Could enhance scoring accuracy by 10-15%")
        else:
            st.info(
                "Current analysis provides comprehensive coverage. For advanced needs, consider GPT-4 for narrative analysis or BERT for cultural context.")
    ...
    def _display_deep_analytics(self, results: Dict) -> None:
        ...
        if component_scores and weights:
            ...
            if cultural_bonus > 0:
                st.success(
                    f"🎉 **Cultural Bonus Applied:** +{cultural_bonus:.3f} points")
    ...
    def _create_score_radar_chart(self, scores: Dict) -> None:
        ...
        try:
            ...
            fig = go.Figure(data=go.Scatterpolar(
                ...
                theta=[cat.replace('_', ' ').title()
                       for cat in categories] + [categories[0].replace('_', ' ').title()],
                fill='toself',
                fillcolor='rgba(102, 126, 234, 0.3)',
                line=dict(color='rgb(102, 126, 234)', width=2),
                hoverinfo='text',
                text=[f"{cat.replace('_', ' ').title()}: {val}/5.0" for cat, val in zip(
                    categories, values)]
            ))
    ...
    def _display_score_breakdown(self, results: Dict) -> None:
        ...
        if scoring_breakdown:
            ...
            with col1:
                ...
                for component, score in component_scores.items():
                    st.progress(score/5,
                                text=f"{component.title()}: {score}/5.0")
            ...
            if len(history) > 1:
                ...
                avg_score = np.mean(scores)
                percentile = np.sum(
                    np.array(scores) < current_score) / len(scores) * 100
                ...
                with col3:
                    st.metric("Position",
                              f"{np.sum(np.array(scores) < current_score) + 1}/{len(scores)}")


# --------------------------
# ENHANCED SIDEBAR COMPONENTS WITH DIAGNOSTICS
# --------------------------
...
def display_enhanced_sidebar() -> None:
    ...
    if st.sidebar.button("🔍 System Diagnostics", width='stretch', key="sidebar_diagnostics"):
    ...
    if films:
        ...
        with st.sidebar.expander("More Stats"):
            ...
            if scores:
                st.sidebar.write(
                    f"**Score Range:** {max(scores) - min(scores):.1f}")
    ...
    if st.session_state.get('diagnostic_results'):
        with st.sidebar.expander("🔍 Last Diagnostic"):
            diag = st.session_state.diagnostic_results
            st.sidebar.write(f"**Status:** {diag['status']}")
            st.sidebar.write(
                f"**Checks:** {diag['checks_passed']}/{diag['total_checks']}")
            if diag.get('issues'):
                st.sidebar.write("**Issues:** " + ", ".join(diag['issues']))
...
def show_diagnostics_page() -> None:
    """Display the diagnostics page."""
    st.header("🔍 System Diagnostics & Health Check")

    col1, col2 = st.columns([2, 1])

    with col1:
        if st.button("🔄 Run Diagnostic Check", type="primary", width='stretch'):
    ...
    if st.session_state.get('diagnostic_results'):
        diagnostics = st.session_state.diagnostic_results

        st.subheader(f"Diagnostic Results: {diagnostics['status']}")

        # Progress bar
        success_rate = (diagnostics['checks_passed'] /
                       diagnostics['total_checks']) * 100
        st.progress(success_rate / 100,
                    text=f"{diagnostics['checks_passed']}/{diagnostics['total_checks']} checks passed ({success_rate:.0f}%)")
        ...
        with col1:
            ...
            st.write(f"- Total checks: {diagnostics['total_checks']}")
            st.write(f"- Passed: {diagnostics['checks_passed']}")
            st.write(f"- Success rate: {success_rate:.0f}%")
        ...
        if success_rate < 90:
            ...
            **System Optimization Recommended:**
            1. Check NLTK data installation
            2. Verify YouTube API connectivity
            3. Clear cache if performance is slow
            4. Consider reducing batch size for large analyses
            """)
        else:
            st.success("""
        ...
        # Quick fixes
        with st.expander("🔧 Quick Fixes", expanded=False):
            st.markdown("""
        ...
        st.info("Click 'Run Diagnostic Check' to analyze system health.")
